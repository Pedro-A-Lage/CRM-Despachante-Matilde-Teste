...
const CUSTO_MATRIX: Record<TipoServico, { dae: 'principal' | 'alteracao' | false; vistoria: boolean | 'se_troca'; placa: 'sempre' | 'se_troca' | 'nunca' }> = {
  transferencia:          { dae: 'principal',  vistoria: true,       placa: 'se_troca' },
  primeiro_emplacamento:  { dae: 'principal',  vistoria: true,       placa: 'sempre' },
  segunda_via:            { dae: 'principal',  vistoria: true,       placa: 'nunca' },
  alteracao_dados:        { dae: 'alteracao',  vistoria: 'se_troca', placa: 'se_troca' },
  baixa:                  { dae: 'alteracao',  vistoria: true,       placa: 'nunca' },
  mudanca_caracteristica: { dae: 'principal',  vistoria: true,       placa: 'nunca' },
  mudanca_categoria:      { dae: 'principal',  vistoria: true,       placa: 'nunca' },
  vistoria_lacrada:       { dae: 'principal',  vistoria: true,       placa: 'nunca' },
  baixa_impedimento:      { dae: 'alteracao',  vistoria: false,      placa: 'nunca' },
};
...
export async function gerarCobrancasIniciais(
  ...
  // Try to load config from service_config table first; fall back to hardcoded matrix
  ...
  try {
    ...
    // Ignore errors from service_config lookup; fall through to hardcoded matrix
  }

  if (!regra) {
    regra = CUSTO_MATRIX[tipoServico];
  }
  if (!regra) return;
...
export async function getAllChargesWithOS(): Promise<ChargeWithOS[]> {
  ...
  for (let i = 0; i < osIds.length; i += BATCH_OS) {
    ...
    const { data, error } = await supabase
      .from('ordens_de_servico')
      .select('id, numero, tipo_servico, cliente_id, veiculo_id, troca_placa, vistoria')
      .in('id', chunk);
    if (error) throw error;
  ...
  // Batch em grupos de 15 para evitar URL muito longa (400 Bad Request)
  const BATCH = 15;

  async function batchIn<T>(table: string, cols: string, ids: string[]): Promise<T[]> {
...
export async function getRelatorioOS(
  ...
  async function batchIn<T>(table: string, cols: string, ids: string[]): Promise<T[]> {
    ...
    for (let i = 0; i < ids.length; i += BATCH) {
      const chunk = ids.slice(i, i + BATCH);
      const { data, error: e } = await supabase.from(table).select(cols).in('id', chunk);
      if (e) throw e;
      if (data) results.push(...(data as T[]));
    }
    return results;
