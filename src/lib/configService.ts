...
export interface ServiceConfig {
  ...
  custosExtras?: Array<{ codigo: string; condicao: 'sempre' | 'se_troca' }>;
  criado_em: string;
...
export async function getAllServiceConfigs(): Promise<ServiceConfig[]> {
  ...
  const configs = (data ?? []).map((row: any) => ({
    ...
    custosExtras: Array.isArray(row.custos_extras) ? row.custos_extras : JSON.parse(row.custos_extras || '[]'),
  })) as ServiceConfig[];
...
export async function updateServiceConfig(
  ...
  updates: Partial<Pick<ServiceConfig,
    ...
    'documentos_extras' | 'dae_tipo' | 'gera_vistoria' | 'gera_placa' | 'custosExtras'
  >>,
): Promise<void> {
  const { custosExtras, ...rest } = updates as any;
  const dbUpdates: any = { ...rest };
  if (custosExtras !== undefined) {
    dbUpdates.custos_extras = custosExtras;
  }
...
export async function createServiceConfig(config: {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      custos_extras: config.custosExtras ?? [],
    })
    .select()
    .single();
  if (error) throw error;
...
export async function gerarChecklistDinamico(
  ...
  if (!config) {
    throw new Error(`Serviço não encontrado nas configurações: ${tipoServico}`);
  }
