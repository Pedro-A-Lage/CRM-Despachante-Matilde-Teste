...
// Agora busca do Supabase (service_config) com fallback hardcoded
// ============================================
...
/**
 * Gera checklist - tenta buscar do Supabase primeiro, fallback para hardcoded.
 * IMPORTANTE: Esta é a versão SÍNCRONA (fallback). Use gerarChecklistAsync para a versão assíncrona.
 */
...
/**
 * Versão ASSÍNCRONA - busca configuração do Supabase.
 * Se o Supabase falhar, faz fallback para a versão hardcoded acima.
 */
export async function gerarChecklistAsync(
    ...
    try {
        ...
        console.warn('Fallback para checklist hardcoded:', err);
        return gerarChecklist(tipoServico, tipoCliente, cpfVendedor);
    }
}
