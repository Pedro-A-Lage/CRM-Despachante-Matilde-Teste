...
import { getClientes, getVeiculosByCliente, getVeiculos, saveOrdem, updateCliente, saveVeiculo, saveCliente } from '../lib/storage';
...
import type { TipoServico, TipoCliente, TipoVeiculo, Cliente, Veiculo } from '../types';
import { getServiceLabels } from '../lib/configService';
import { getServicePrice } from '../lib/financeService';
...
export default function OSForm({ drawerMode = false, onCreated, onCancel, initialExtensionData }: OSFormProps = {}) {
    ...
    // Merged service labels: hardcoded as base, Supabase service_config overrides/extends
    ...
    useEffect(() => {
        getServiceLabels()
            .then(dynamicLabels => {
                setServiceLabels({ ...TIPO_SERVICO_LABELS, ...dynamicLabels });
            })
            .catch(() => {
                // Fall back to hardcoded labels if Supabase is unavailable
            });
    }, []);
    ...
    const handleConfirmPdfReview = async () => {
        ...
        try {
            ...
            const { getClientes, saveCliente, getVeiculos, saveVeiculo } = await import('../lib/storage');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const checklist = await gerarChecklistAsync(
            ...
            try {
                ...
                console.error('Aviso: cobranças automáticas não geradas:', err);
            }
                    ...
                    {selectedCliente && (
                        <div style={{
                            ...
                            <ul style={{ paddingLeft: '20px', margin: 0, color: 'var(--color-text-secondary)', fontSize: '0.9rem' }}>
                                {gerarChecklist(tipoServico, selectedCliente.tipo, cpfVendedor.trim() || undefined).map((item) => (
