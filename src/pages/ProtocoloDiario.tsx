...
import { getOrdens, getClientes, getVeiculos, saveProtocolo, getProtocolos } from '../lib/storage';
...
export default function ProtocoloDiario() {
    ...
    const imprimirProtocolo = () => {
      ...
      <body>
        <div class="header">
          <img src="${window.location.origin}/logo.png" alt="Logo" style="height: 100px;" />
        ...
        <table>
          ...
          <tbody>
            ${protocolo.processos.map((p: any) => `
              <tr>
                <td>${p.clienteNome}</td>
                <td>${p.veiculoPlaca || ''}</td>
                <td style="font-size:9px">${p.veiculoChassi || ''}</td>
                <td>${p.veiculoRenavam || ''}</td>
                <td>${TIPO_SERVICO_LABELS[p.tipoServico as TipoServico] || p.tipoServico}</td>
                ...
                <div style={{
                    ...
                    {protocoloHoje.processos.length === 0 ? (
                        ...
                        <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 8 }}>
                            {protocoloHoje.processos.map((p: any, idx: number) => {
                                        ...
                                        <div style={{ flex: 1, minWidth: 0 }}>
                                            ...
                                            <div style={{
                                                ...
                                                <span style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                                                    <FileText size={10} /> {TIPO_SERVICO_LABELS[p.tipoServico as TipoServico] || p.tipoServico}
                                                </span>
                                            </div>
                                        </div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10 }}>
                            ...
                            <div>
                                ...
                                <select className="form-select" value={mServico} onChange={(e) => setMServico(e.target.value as TipoServico)}
                                    style={{ fontSize: 12, padding: '7px 10px', borderRadius: 8 }}>
                                    {(Object.entries(TIPO_SERVICO_LABELS) as [TipoServico, string][]).map(([k, v]) => (
                                ...
                                {manualProcessos.map((p, i) => (
                                    <div key={i} style={{
                                        ...
                                        <span style={{ fontSize: 10, color: 'var(--color-text-tertiary)' }}>
                                            {TIPO_SERVICO_LABELS[p.tipoServico as TipoServico] || p.tipoServico}
                                        </span>
