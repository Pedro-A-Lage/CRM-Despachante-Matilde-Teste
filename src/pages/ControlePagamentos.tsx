...
function ConfirmPopover({ charge, onClose, onConfirm }: ConfirmPopoverProps) {
  const [valor, setValor] = useState<string>(
    maskMoney(String(Math.round((isNaN(charge.valor_previsto) ? 0 : charge.valor_previsto) * 100)).padStart(3, '0')),
  );
...
function OSCard({ group, isAdmin, onConfirm, onRevert, onConfirmAll, loadingConfirm }: OSCardProps) {
  ...
  const tipoLabel =
    TIPO_SERVICO_LABELS[group.tipoServico as keyof typeof TIPO_SERVICO_LABELS] ?? group.tipoServico;
