  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, maxWidth: 900 }}>
      ...
      <div style={{
        ...
        border: `1px solid ${isQuitado ? 'rgba(16,185,129,0.3)' : faltaReceber > 0 ? 'rgba(239,68,68,0.3)' : 'var(--border-color)'}`,
        background: isQuitado ? 'rgba(16,185,129,0.05)' : faltaReceber > 0 ? 'rgba(239,68,68,0.04)' : 'var(--bg-body)',
      }}>
        <div style={{ padding: '14px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            {/* Status badge */}
            <div style={{
              padding: '6px 14px', borderRadius: 8, fontWeight: 800, fontSize: 13, letterSpacing: 0.5,
              background: isQuitado ? 'rgba(16,185,129,0.15)' : faltaReceber > 0 ? 'rgba(239,68,68,0.12)' : 'rgba(245,158,11,0.12)',
              color: isQuitado ? 'var(--color-success)' : faltaReceber > 0 ? 'var(--color-danger)' : 'var(--color-primary)',
              display: 'flex', alignItems: 'center', gap: 5,
            }}>
              {isQuitado ? <><IconCheck /> QUITADO</> : faltaReceber > 0 ? 'DEVENDO' : 'EM DIA'}
            </div>
            {/* Progress bar */}
            <div style={{ width: 100, height: 6, borderRadius: 3, background: 'rgba(128,128,128,0.15)', overflow: 'hidden', flexShrink: 0 }}>
              <div style={{
                height: '100%', borderRadius: 3, width: `${progressoPct}%`,
                background: isQuitado ? 'var(--color-success)' : progressoPct > 0 ? 'var(--color-primary)' : 'transparent',
                transition: 'width 0.5s ease',
              }} />
            </div>
            <span style={{ fontSize: 13, fontWeight: 800, color: isQuitado ? 'var(--color-success)' : 'var(--color-primary)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 10, fontWeight: 600, color: 'var(--color-text-tertiary)', textTransform: 'uppercase' }}>Serviço</div>
              <div style={{ fontSize: 15, fontWeight: 800, color: 'var(--color-primary)' }}>{fmt(valorServicoEfetivo)}</div>
            </div>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 10, fontWeight: 600, color: 'var(--color-text-tertiary)', textTransform: 'uppercase' }}>Recebido</div>
              <div style={{ fontSize: 15, fontWeight: 800, color: 'var(--color-success)' }}>{fmt(totalRecebido)}</div>
            </div>
            {faltaReceber > 0 && (
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 10, fontWeight: 600, color: 'var(--color-text-tertiary)', textTransform: 'uppercase' }}>Falta</div>
                <div style={{ fontSize: 15, fontWeight: 800, color: 'var(--color-danger)' }}>{fmt(faltaReceber)}</div>
              </div>
            )}
            {descontoValor > 0 && (
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 10, fontWeight: 600, color: 'var(--color-text-tertiary)', textTransform: 'uppercase' }}>Desconto</div>
      ...
      <div style={{ background: 'var(--bg-body)', borderRadius: 10, border: '1px solid var(--border-color)', overflow: 'hidden' }}>
        <div style={{
          ...
          <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--color-primary)', opacity: 0.7, marginRight: 8 }}>
          ...
          {!readOnly && (
            <button
              ...
              style={{
                ...
                background: 'var(--color-primary)', color: 'var(--bg-body)',
