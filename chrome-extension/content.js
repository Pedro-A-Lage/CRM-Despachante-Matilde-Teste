...
function modVistoriaECV() {
    ...
    if (isAgendamento) {
        ...
        // Salva contexto para os próximos passos
        // IMPORTANTE: só salva matilde_osId se estiver presente na URL —
        // não sobrescrever com string vazia o osId já salvo pelo background/crm_bridge
        if (osId || placa || chassi) {
            const ctx = {};
            if (osId) ctx.matilde_osId = osId; // só salva se presente
            if (placa) ctx.matilde_placa = placa;
            if (chassi) ctx.matilde_chassi = chassi;
            if (cpfCnpj) ctx.matilde_cpfCnpj = cpfCnpj;
            ctx.matilde_servico_ativo = 'vistoria_ecv';
            saveContext(ctx);
        }
