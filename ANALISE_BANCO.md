...
|---|----------|-----------|--------|--------|
| 1 | `desconto` usado no código mas ausente no schema | **Crítico** | `ordens_de_servico` | **RESOLVIDO** (migration 20260324000000) |
| 2 | FK ausente em `finance_charges.os_id` | **Crítico** | `finance_charges` | **RESOLVIDO** (migration financeiro_module criou FK) |
| 3 | FK ausente em `payments.os_id` | **Crítico** | `payments` | **RESOLVIDO** (migration financeiro_module criou FK) |
| 4 | FK ausente em `payments.charge_id` | Alta | `payments` | **RESOLVIDO** (migration fix_payments_nullable_charge) |
| 5 | Coluna `financeiro` JSONB obsoleta | Alta | `ordens_de_servico` | Pendente — verificar se vazia e remover |
| 6 | Dados duplicados: vistoria em JSONB e colunas escalares | Média | `ordens_de_servico` | Pendente |
| 7 | Nenhum índice secundário em nenhuma tabela | Alta | Todas | **PARCIAL** (índices financeiros criados, faltam os demais) |
...
| 11 | Sem trigger de `atualizado_em` | Baixa | Todas | **PARCIAL** (triggers financeiros criados) |
